...
loaded, already_present, skipped, failed = [], [], [], []
last_family = None
total = len(picks)
    ...
    for i, (name, path) in enumerate(picks, 1):
        ...
        try:
            if not os.path.isfile(path):
                raise IOError("File not found on disk: {0}".format(path))

            # Check up front whether a family with this name is already in
            # the project. LoadFamily returns False (not an error) when the
            # family on disk is identical to what's already loaded - there's
            # nothing new to bring in, so that's a success case, not a
            # failure, and we still want to hand back a usable Family object.
            existing_fam = None
            ...
            if ok:
                ...
                if fam is not None:
                    last_family = fam
            elif existing_fam is not None:
                already_present.append(name)
                last_family = existing_fam
            else:
                skipped.append((name, "LoadFamily returned False (the file "
                                       "may not be a valid family)."))
        except Exception as ex:
...
placed_prompt = False
if len(loaded) + len(already_present) == 1 and last_family is not None:
    try:
...
if loaded:
    lines.append(u"  - " + u"\n  - ".join(loaded))
if already_present:
    lines.append(u"Already up to date in the project: {0}".format(len(already_present)))
    lines.append(u"  - " + u"\n  - ".join(already_present))
if skipped:
    lines.append(u"Skipped: {0}".format(len(skipped)))
    for n, r in skipped:
        lines.append(u"  - {0}: {1}".format(n, r))
if failed:
...
forms.alert(u"\n".join(lines), title="Import Family", warn_icon=bool(failed))
