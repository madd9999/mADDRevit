...
if isinstance(view, DB.ViewSchedule) or view.ViewType in [DB.ViewType.Legend, DB.ViewType.Rendering, DB.ViewType.DraftingView, DB.ViewType.ProjectBrowser]:
    forms.alert("Open a model view (plan/section/elevation/3D).", title="Tag Doors", exitscript=True)
...
# Target categories
cats = [DB.BuiltInCategory.OST_Doors]
...
if not targets:
    forms.toast("No target elements visible.", title="Tag Doors", appid='TagSuite')
    sys.exit()
...
t = DB.Transaction(doc, "Tag Doors")
t.Start()
created = 0
skipped = 0
errors = 0
...
forms.toast("Tagged: {} | Skipped: {} | Errors: {}".format(created, skipped, errors), title="Tag Doors", appid='TagSuite')
